...
| H-7 | HIGH | No privacy policy, no terms of use, no signup acceptance — the product collects PII (name, email, phone, portfolio) and publishes creative work with zero legal surface (owner's note 2026-08-02) | Two layers: (1) interim plain-English privacy/data notice + "by joining you agree" line in both signups — can ship immediately on owner sign-off; (2) real ToS + privacy policy drafted WITH HMNTY's counsel, then a post-signup acceptance step **aligned per user type** — talent terms (media display/licensing after moderation, public portfolio links, no work guarantee) differ from business terms (seat responsibility, conduct toward creatives, off-platform introduction terms). Acceptance should be recorded (userId, docVersion, timestamp) | S interim / M full |
...
2. **Before real-user growth (next)** — H-7 legal surface first (interim notice
   immediately on owner sign-off; full per-role ToS/privacy with HMNTY's
   counsel + versioned acceptance step), then H-1 (availability first), H-3,
   H-5, H-2 decision, H-4, H-6; DB-backed AI usage counter replacing the
   in-memory cap; AI budget number decided by Omar + founders.
